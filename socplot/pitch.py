...
class Pitch:
    ...
    def plot_pass(self, start, end, match_id, team , timeMax, timeMin, pass_type='Ground Pass',color=''):
        """
        ...
        if not color:
            color = 'orange'
        if pass_type == 'Low Pass':
            line = 'D'
        elif pass_type == 'Ground Pass':
            line = 'o'
        ...
        self._plt.plot(end[0], end[1], line, color='green')
        self._plt.title('Pass Map for match {0} \n Team={1}, Between minute {2} and {3} '.format(match_id,team,timeMin,timeMax))
    ...
    def heat_map(self, xVals, yVals, color='red'):
        """
        ...
        None
        """
        sns.kdeplot(xVals, yVals, shade=True, n_levels=20, color=color)
